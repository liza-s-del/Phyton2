...
class FloorPlan:
    ...
    def __init__(self, total_area: float, rooms: List[Dict]):
        ...
        if total_rooms_area > self.total_area:
            raise ValueError(
                f"Суммарная площадь комнат ({total_rooms_area:.2f} м²) "
                f"превышает общую площадь ({self.total_area:.2f} м²)"
            )
    ...
    def add_room(self, room_name: str, area: float) -> None:
        ...
        if current_total + area > self.total_area:
            ...
            raise ValueError(
                f"Недостаточно площади. Доступно: {available:.2f} м², "
                f"требуется: {area:.2f} м²"
            )
    ...
    def get_total_rooms_area(self) -> float:
        ...
        :return: Суммарная площадь комнат в м²
